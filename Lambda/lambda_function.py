    ...
    if age is not None:
        ...
        if age < 0:
            return build_validation_result(
                False,
                "age",
                "Please input a valid age that is equal to / greater than 0 years, "
            ...
            return build_validation_result(
                False,
                "age",
                "Please input a valid age that is less than 65 years, "
                "please provide a different age.",
            )    
    ...
    if investment_amount is not None:
        ...
        if investment_amount < 5000:
            return build_validation_result(
                False,
                "investment_amount",
                "The amount to invest should be greater than or equal to five thousand, "
                "please provide a correct amount in dollars to convert.",
            )
            
    # A True results is returned if age or amount are valid (To be reviewed)
...
# Gets the invocation source, for Lex dialogs "DialogCodeHook" is expected.
    ...
    # Display the portfolio options based on risk level.
    
    risk_level = get_slots(intent_request)["riskLevel"]
    if risk_level == 'None':
        portfolio1 = "100% bonds (AGG), 0% equities (SPY)"
    elif risk_level == 'Low':
        portfolio1 = "60% bonds (AGG), 40% equities (SPY)"
    elif risk_level == 'Medium':
        portfolio1 = "40% bonds (AGG), 60% equities (SPY)"
    elif risk_level == 'High':
        portfolio1 = "20% bonds (AGG), 80% equities (SPY)"
    
    #portfolio = ()
    portfolio = (portfolio1)
    

    # Return a message with the program's result.
    return close(
        intent_request["sessionAttributes"],
        "Fulfilled",
        {
